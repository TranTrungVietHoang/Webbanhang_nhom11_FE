...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    console.log('[Login] Submitting form with:', formData);
    try {
      const response = await axios.post('http://localhost:3000/api/auth/login', formData);
      console.log('[Login] Response:', response.data);
      toast.success(response.data.message);
      
      // Dùng context để set user & token
      if (response.data.token && response.data.user) {
        console.log('[Login] Calling login() with token and user');
        login(response.data.token, response.data.user);
        console.log('[Login] Login called, navigating to /');
      }
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-gray-900 border-r border-gray-200">
        ...
        <div className="relative z-10 flex flex-col justify-center px-16 text-white max-w-2xl">
          ...
          <h1 className="text-5xl font-bold mb-6 leading-tight">Chào mừng trở lại!</h1>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 bg-slate-500">
        ...
        >
          <div className="mb-10 text-center lg:text-left">
            <h2 className="text-4xl font-bold text-gray-900 mb-3 tracking-tight">Đăng nhập</h2>
            <p className="text-gray-500 font-medium">Nhập thông tin để truy cập tài khoản</p>
          </div>
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  name="password"
                  required
                  className="block w-full pl-12 pr-12 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 focus:bg-white transition-all outline-none"
                  placeholder="••••••••"
                  onChange={handleChange}
                />
                <button
            ...
            <div className="flex items-center justify-between py-1">
              <label className="flex items-center space-x-3 cursor-pointer group">
                <div className="relative">
                  ...
                  <div className="w-5 h-5 border-2 border-gray-300 rounded-md peer-checked:bg-blue-600 peer-checked:border-blue-600 transition-all group-hover:border-blue-300"></div>
                ...
                <span className="text-sm font-semibold text-gray-600 group-hover:text-gray-900 transition-colors">Ghi nhớ đăng nhập</span>
              </label>
              <Link to="/forgot-password" title="Quên mật khẩu?" className="text-sm font-bold text-blue-600 hover:text-blue-700 transition-colors">
                Quên mật khẩu?
              </Link>
            </div>
          ...
          <p className="mt-8 text-center text-gray-500 font-medium font-sans">
            Chưa có tài khoản?{' '}
          ...
          <div className="relative my-10">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="px-4 bg-white text-gray-400 font-bold uppercase tracking-widest">Hoặc đăng nhập với</span>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <button className="flex items-center justify-center py-3.5 px-4 bg-white border border-gray-200 rounded-2xl hover:bg-gray-50 hover:border-gray-300 transition-all active:scale-[0.98] shadow-sm">
